    ...
    for num=nums
        for ratio=ratios
            for i=1:length(dialects)
                dialect = dialects{i};
                dataset = {basedir, 'codes', '/', 'denoising', '/', 'Data', '/', 'TIMIT', '/', 'TRAIN', '/', char(dialect)};
                ...
                for j=3:length(dataset_arr)
                    ...
                    save_dir = {base_save, 'las', '/', 'denoised_train', '/', num2str(num), '/', 'TIMIT', '/', 'TRAIN', '/', char(dialect), '/', speaker.name};
                    ...
                    wav_noise = stft2( z filesep, 'denoising']);
